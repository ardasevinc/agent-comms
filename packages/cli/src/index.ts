#!/usr/bin/env bun
import { serveChannel } from "@agent-comms/channel";
...
program
	.command("channel")
	.description("Claude Code channel bridge commands")
	.command("serve")
	.description("Run the local MCP stdio bridge for Claude Code channels")
	.action(async () => {
		await serveChannel();
	});
