...
import type { AgentIdentity, Message } from "@agent-comms/shared/types";
...
const markMessageReadStmt = db.prepare<void, [number]>(`
  UPDATE messages SET read_at = datetime('now')
  WHERE id = ? AND direction = 'human_to_agent'
`);
...
const getByIdStmt = db.prepare<Message, [number]>(`
  SELECT id, session_id as sessionId, agent_type as agentType, hostname, project,
         direction, content, telegram_message_id as telegramMessageId,
         created_at as createdAt, read_at as readAt
  FROM messages
...
const getRepliesAfterIdStmt = db.prepare<Message, [string, number]>(`
  SELECT id, session_id as sessionId, agent_type as agentType, hostname, project,
         direction, content, telegram_message_id as telegramMessageId,
         created_at as createdAt, read_at as readAt
  FROM messages
  WHERE session_id = ? AND direction = 'human_to_agent' AND id > ?
  ORDER BY id ASC
`);
...
export function markReplyRead(messageId: number): void {
...
export function getMessageById(messageId: number): Message | null {
	return getByIdStmt.get(messageId) ?? null;
}

export function getRepliesAfterId(
	sessionId: string,
	lastEventId: number,
): Message[] {
	return getRepliesAfterIdStmt.all(sessionId, lastEventId);
}
